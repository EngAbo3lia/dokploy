...
import { getComposePath, writeDomainsToCompose } from "../docker/domain.js";
import { encodeBase64, getEnvironmentVariablesObject, prepareEnvironmentVariables, prepareEnvironmentVariablesForFile, } from "../docker/utils.js";
import { withResolvedVaultRefs } from "../vault/index.js";
export const ROLLBACK_OK_MARKER = "__DOKPLOY_ROLLBACK_OK__";
export const getBuildComposeCommand = async (rawCompose, deploymentId) => {
...
const sanitizeCommand = (command) => {
    ...
    if (UNSAFE_COMPOSE_COMMAND.test(sanitizedCommand)) {
        throw new Error("Invalid characters in compose command: shell control characters are not allowed");
    }
    if (sanitizedCommand.includes("&")) {
        ...
        const isSafeChain = chains
            .slice(1)
            .every((cmd) => cmd.startsWith("docker compose ") ||
...
export const getCreateEnvFileCommand = (compose) => {
    ...
    const envFileContent = (compose.composeType === "stack"
        ? prepareEnvironmentVariables(envContent, compose.environment.project.env, compose.environment.env)
        : prepareEnvironmentVariablesForFile(envContent, compose.environment.project.env, compose.environment.env)).join("\n");
